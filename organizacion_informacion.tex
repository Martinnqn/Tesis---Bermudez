...
\section{Diseño}
Para alcanzar esta estructura, se crearon una serie de módulos que transforman la ontología de entrada en un árbol con toda la información contenida en la misma. El primer módulo recibe la entrada y la traduce a una representación interna de la ontología, luego esta representación es enviada a otros dos módulos: uno se encarga de extraer las entidades más relevantes; el otro, con la representación interna de la ontología y las entidades más relevantes, se encarga de generar un árbol de entidades. Para finalizar, un último módulo agrega en una nueva rama del árbol a aquellas entidades que no han sido utilizadas en ninguna rama otra rama.

En la Figura~\ref{fig:modulos_organizador_inf} se muestra la transformación de la ontología OWL a través de los módulos.
