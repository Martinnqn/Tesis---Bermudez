...
Para identificar las entidades más relevantes, usaremos la las medidas de centralidad de los nodos.
...
\begin{itemize}
    ...
    \item El recorrido del grafo para obtener las unidades de información adecuada que maximicen la relación semántica respecto a los tópicos del texto (para abordar la cohesión global).% En este caso, las unidades de información que se tienen en cuenta para el recorrido del grafo son \emph{rdfs:subClassOf}, \emph{owl:ObjectProperty} y \emph{owl:DataProperty}.
\end{itemize}
...
Dado una entidad \emph{E}, para recuperar la información que la describe, adoptaremos como enfoque recorrer la estructura de grafo asociada a la ontología, navegando por las relaciones de \emph{E}. Es necesario definir cómo recorrer el grafo, ya que cada recorrido genera un resultado diferente. 

Como hipótesis, consideramos que para mantener una coherencia local, es necesario recorrer el grafo en anchura, obteniendo la máxima cantidad de información posible acerca de \emph{E}, de manera que el foco de atención solo salte hacia otras entidades cuando no haya nada más para decir acerca de \emph{E}.


En el nivel micro, debemos definir una traducción entre la gramática del lenguaje OWL y del lenguaje humano. Luego se modificará convenientemente la traducción para alcanzar un texto más fluido.